    ...
    if message.chat.type == 'private':
        ...
        if data[0] == "main_menu":
            ...
            if message.text == "🤝Підтримати петицію":
                bot.send_message(message.from_user.id, 'Дана функція тимчасово виключена')
        ...
        if data[0] == 'spec_select':
            if message.text == 'Повідомити про проблему':
                ...
                bot.send_message(message.from_user.id,
                                 '⬇️Нижче ви можете повідомити про проблему та почати зміни уже зараз',
                                 reply_markup=remove_keyboard)
                cursor.execute("INSERT INTO petitions(author, views) VALUES('" + str(message.from_user.id) + "','1')")
                cursor.execute(
                    "SELECT MAX(petition_id) FROM petitions WHERE author='" + str(message.from_user.id) + "'")
                petition_id = cursor.fetchone()[0]
                cursor.execute(
