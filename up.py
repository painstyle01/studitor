...
main_menu.row("🤝Підтримати петицію")
...
def call(message):
    if message.chat.id == admin_chat_id:
        ...
        cursor.execute("UPDATE users SET call='" + str(message.from_user.id) + "' WHERE id=" + str(id))
        cursor.execute("UPDATE users SET call='" + str(id) + "' WHERE id=" + str(message.from_user.id))
        ...
        try:
            ...
            cursor.execute("UPDATE users SET call='0' WHERE id=" + str(id))
            cursor.execute("UPDATE users SET call='0' WHERE id=" + str(message.from_user.id))
        ...
        if message.chat.type == 'private':
            ...
            if data[0] == "main_menu":
                ...
                if message.text == "🤝Підтримати петицію":
                    bot.send_message(message.from_user.id, 'Дана функція тимчасово відключена.')
